...
class LSTM(object):
	...
	def step_forward(self, x, prev_h, prev_c):
		...
		meta = (i, f, o, g, t, prev_c, prev_h, Wx, Wh, x)
		##############################################################################
		#							   END OF YOUR CODE							     #
		##############################################################################
	...
	def step_backward(self, dnext_h, dnext_c, meta):
		...
		#############################################################################
		i, f, o, g, t, prev_c, prev_h, Wx, Wh, x = meta
		do = t * dnext_h
		dt = o * (1 - t * t) * dnext_h + dnext_c
		df = prev_c * dt
		di = g * dt
		dprev_c = f * dt
		dg = i * dt
	...
	def forward(self, x, h0):
		...
		for t in range(T):
			xt = x[:, t, :]
			next_h, next_c, meta = self.step_forward(xt, prev_h, prev_c)
			prev_h = next_h
			prev_c = next_c
	...
	def backward(self, dh):
		...
		#############################################################################
		N, T, H = dh.shape
		i, f, o, g, t, prev_c, prev_h, Wx, Wh, x = self.meta[T - 1]
		D = x.shape[1]
		...
		for t in range(T):
			...
			dnext_c = dprev_c
			dx[:, t, :], dprev_h, dprev_c, dWxt, dWht, dbt = self.step_backward(dnext_h, dnext_c, meta)
			dWx = dWx + dWxt
			dWh = dWh + dWht
			db = db + dbt
		dh0 = dprev_h
